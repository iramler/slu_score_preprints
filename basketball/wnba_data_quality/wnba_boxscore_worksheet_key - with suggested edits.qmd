...
filtered <- reg_season %>% 
  filter(
    season %in% c(2007, 2009, 2010, 2013, 2014, 
...
# wnba_win_data <- full_join(
#   team_wins, half_wins,
#   by = c("team_id", "season")) %>% 
#  full_join(playoffs, by = c("team_id", "season")) %>% 
#  arrange(season) %>%
#  as.data.frame()
...
## find the teams each season that exist in the playoff data
playoff.teams = playoffs |> 
    select(season, team_id, season_type) |> 
    distinct()
playoff.teams

## join team_wins and half_wins
## also add a column to indicate whether that team made the playoffs
wnba_win_data = full_join(
  team_wins, half_wins,
  by = c("team_id", "season")) %>% 
 left_join(playoff.teams, by = c("team_id", "season")) %>% 
 arrange(season) %>%
 as.data.frame()

wnba_win_data <- wnba_win_data %>% 
  mutate(playoff = ifelse(season_type == 3, 'Make Playoffs', 'Missed Playoffs')) %>%
  mutate(playoff = ifelse(is.na(playoff), 'Missed Playoffs', playoff))
...
wnba_win_data <- wnba_win_data %>% 
  mutate(half_top8 = ifelse(half_rank <= 8, "Top8atMid", "NotTop8atMid"))
```

```{r}
# will want to clean up column names 
# if not manually recreating table
final.table = wnba_win_data %>%
  group_by(half_top8) %>%
  count(playoff) %>% 
